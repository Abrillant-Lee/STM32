...
void Encoder_TIM2_Mode_Config(void)
{
    ...
    TIM_TimeBaseStructure.TIM_Period            = 65535;              // 自动重装载寄存器的值，累计TIM_Period+1个频率后产生一个更新或者中断
    ...
    TIM_ClearFlag(TIM2, TIM_FLAG_Update);      // 清除定时器溢出更新标志位(清除计数值)
    TIM_ITConfig(TIM2, TIM_IT_Update, ENABLE); // 定时器2，溢出更新，使能
    TIM_SetCounter(TIM2, 0);                   // 定时数据清零(输入捕获的值从0开始计数)
...
{
    int8_t motor_direction    = 0;    /* 电机旋转方向 */
    int32_t capture_count     = 0;    /* 当前时刻总计数值 */
    static int32_t last_count = 0;    /* 上一时刻总计数值 */
    int32_t count_per_unit    = 0;    /* 单位时间内总计数值 */
    float shaft_speed         = 0.0f; /* 电机转轴转速 */
    float number_of_rotations = 0.0f; /* 累积圈数 */
    /* 20ms计算一次 */
    motor_direction     = (TIM1->CR1 & TIM_CR1_DIR) ? -1 : 1;                    /* 电机旋转方向 = 计数器计数方向 */
    capture_count       = TIM1->CNT + (Encoder_Overflow_Count * 65535);          /* 当前时刻总计数值 = 计数器值 + 计数溢出次数 * ENCODER_TIM_PERIOD */
    count_per_unit      = capture_count - last_count;                            /* 单位时间内总计数值 = 当前时刻总计数值 - 上一时刻总计数值 */
    shaft_speed         = (float)count_per_unit / ENCODER_TOTAL_RESOLUTION * 50; /* 转轴转速 = 单位时间内的计数值 / 编码器总分辨率 * 时间系数 */
    number_of_rotations = (float)capture_count / ENCODER_TOTAL_RESOLUTION;       /* 累积圈数 = 当前时刻总计数值 / 编码器总分辨率 */
    last_count          = capture_count;                                         /* 记录当前总计数值，供下一时刻计算使用 */
    // printf("\r\n电机方向：%d\r\n", motor_direction);
